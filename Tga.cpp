...
unsigned char LoadUncompressedTGA(Texture * texture, const char * filename, FILE * fTGA)	/* Load an uncompressed TGA (note, much of this code is based on NeHe's */
{
...
unsigned char LoadCompressedTGA(Texture * texture,const char * filename, FILE * fTGA)		/* Load COMPRESSED TGAs */
{
	...
	do
	{
		...
		else																			/* chunkheader > 128 RLE data, next color  reapeated chunkheader - 127 times */
		{
			...
			{																			/* by the header */
				...
				{
					printf("Error too many pixels read (curr:%d; pixcount:%d",currentpixel,pixelcount);								/* if there is too many... Display an error! */
