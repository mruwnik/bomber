...
class Board{
	...
	int n,m,players,baddys,over,gameType; // wysokosc, szerokosc planszy; ile zostalo graczy, stworow; czy juz ktos wygral
	bool alive[4];		// tablica flag ktora okresla ktore gracze jeszcze zyja
	Wall wall;		// egzemplarz sciany niezniszczalnej ktora jest na plaszy
	Texture floor;		// tekstura podlogi
	List<object> list;	// lista ( w sumie wektor ) objektow aktywnych
	Board(const Board&){};	// aby nie dalo sie skopiowac tablice
public:
	Board(int n, int m);	
	void setWall(Texture*); // Ustawia teksture scian niezniszczalnych 
	void newBoard(int n, int m);
	...
	void clear();		// czysci plansze
	void playerDown(int nr);// dekrementuje zmienna players przy smierci gracza i oznacza go jako martwy
	void baddyDown();	// to samo dla stworkow
	int checkBaddys();	// zwraca ilosc stworow na plansie
	int end(int x, int y);	// konczy gre na obecnej plansie
...
public:
	...
	action check(action); 	// sprawdza czy pole w danym kierunku jest wolne
	friend class initialiser; // klasa ktora inicjalizuje plansie
	~pos();
};
#endif
